...
const Checkout = () => {
  ...
  const createOrder = async ({ name, lastname, phone, email }) => {
    try {const objOrder = {
      ...
      items: cart,
      total: total,
      fecha: Timestamp.fromDate(new Date()), // Reemplazar objOrder.data con objOrder.fecha
    };
      ...
      if (outOfStock.length === 0) {
        ...
        const ordersQuery = query(orderRef, where('fecha', '==', objOrder.fecha), where('nombre', '==', objOrder.buyer.name)); // Utilizar objOrder.fecha y objOrder.buyer.name
